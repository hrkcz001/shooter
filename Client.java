			...
			if(!dataForPaint.isEmpty()){
				...
				if(!splitedData[1].isEmpty()){
					...
					for(int i = 0; i < playersString.length; i++){
						...
						g.fillOval((int)(df.parse(playersString[i].split("/")[0]).doubleValue() * kWidth), (int)(df.parse(playersString[i].split("/")[1]).doubleValue() * kHeight), (int)(50 * kWidth), (int)(50 * kHeight));
...
class MainPane extends JLayeredPane{
  ...
  public MainPane(Wind wind, Dimension size) throws Exception{
	...
	public void createServerTable(){
		...
		Boolean connectionRes = wsc.setSocket("192.168.4.115",6900);
...
class ServerConnectionThread extends Thread{
	...
	public void run(){
		...
		try{
			...
			GameServerConnection gsc = new GameServerConnection("192.168.4.115", port);
	...
	public void run(){
		...
		try{
			...
			while(sct.started){
				...
				System.out.println(sct.w + " " + sct.a + " " + sct.s + " " + sct.d);
